...
```bash
# Create containers only (no start)
./start_stack.sh create
...
# Delete containers and network (preserves volumes)
./start_stack.sh delete

# Show status
./start_stack.sh status
```

`start_stack.sh` manages container lifecycle using the files generated by `build_docker.sh` (`.env` and `docker-compose-thislab.yml`).
